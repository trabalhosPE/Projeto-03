...
struct dados
{ // variaveis dos dados do cadastro.
  int id;
...
struct noLista
{ // variaveis da lista simples encadeada.
  struct dados pessoa;
  struct noLista *prox;
};
...
void listaCadastro(struct noLista *imprimir)
{
  ...
  while (ptr != NULL)
  {
    ...
    printf("Email: %s\n", ptr->pessoa.email);
    printf("Sexo: %s\n", ptr->pessoa.genero);
    printf("Endereço: %s, ", ptr->pessoa.end.cep);
    printf("%s, ", ptr->pessoa.end.bairro);
    printf("%s - ", ptr->pessoa.end.cidade);
    printf("%s", ptr->pessoa.end.estado);
